...
| `spin_noise_driver.py` | External session driver — the tested form of Tier 2. Prompts the operator, writes `answers.json` itself, then drives VnmrJ block-by-block through `listenon`/`send2Vnmr`, waiting for each `.fid` to land. Python 2.6 stdlib only. Unlike `spin_noise_run.mac` it runs *outside* VnmrJ, so it needs no `wexp` self-chaining and no created parameters surviving between blocks. **Exercised on hardware:** the default session and one bracketed ladder (SIU DD2 / VnmrJ 3.2, sessions 2–3), plus a 966-block 10 h run, all with zero failures. **Not yet exercised on a console:** randomized ladder, tuning ladder, `--tof-test`, and the retry / timeout / disarm paths. Leaves the experiment disarmed (`wexp=''`) via `try/finally`, including on Ctrl-C. Shim on the tube you are about to run and check the linewidth first — see its PRECONDITIONS; a near-neat H₂O sample cannot diagnose a shim at all. `--dry-run` prints every command and sends nothing. |
