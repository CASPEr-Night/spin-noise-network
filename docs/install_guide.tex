...
\begin{itemize}
\item \textbf{A Bruker spectrometer running \ts{} 2.x, 3.x, or 4.x.} The
...
\item a dataset \file{SPINNOISE\_<yyyymmdd>} exists with expnos
...
\item the bundle zip,
      \file{SPINNOISE\_<date>/}\allowbreak\file{spinnoise\_<slug>\_<YYYYMMDD\_HHMMSS>Z\_<hex>.zip},
...
\item \dialog{Noise block starting} --- shows rows, seconds per row, total
      time, and RG. The pulse-free block starts when you close it ---
      \emph{this is the walk-away point}.
...
The run creates one dataset, \file{SPINNOISE\_<date>}, in your current data
directory:
...
and reads and writes nothing outside the \file{SPINNOISE\_<date>} dataset
...
inside the \file{SPINNOISE\_<date>} dataset directory as
...
\item \textbf{A hardware command hangs with no progress for over 5 minutes}
      ...
      its own and all data stays in \file{SPINNOISE\_<date>}.
\item \textbf{A Jython error dialog appears} --- the run stopped, but started
      acquisitions finish and the data stays in \file{SPINNOISE\_<date>}.
